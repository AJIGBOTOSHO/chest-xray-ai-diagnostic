...
def make_gradcam_heatmap(img_array, new_base, classifier_layers, pred_index):
    with tf.GradientTape() as tape:
        # Forward through base → feature maps
    ...
    conv_features = conv_features[0]                            # (7,7,1024)
    heatmap = tf.reduce_sum(tf.multiply(pooled_grads, conv_features), axis=-1)
...
def heatmap_to_bbox(heatmap, original_shape, threshold=0.5):
    ...
    _, binary = cv2.threshold(np.uint8(255 * heatmap_big), int(threshold * 255), 255, cv2.THRESH_BINARY)
...
def create_overlay(original_pil, heatmap, pred_class, confidence, threshold=0.5):
    ...
    heatmap_big = (heatmap_big - heatmap_big.min()) / (heatmap_big.max() - heatmap_big.min() + 1e-10)
    heatmap_color = cv2.applyColorMap(np.uint8(255 * heatmap_big), cv2.COLORMAP_JET)
    ...
    if bbox:
        ...
        cv2.putText(overlay, f"{pred_class} ({confidence:.2%})", (x, y-10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
    return Image.fromarray(overlay), bbox
...
if uploaded:
    ...
    if show_gradcam:
        ...
        if heatmap is not None:
            overlay_img, bbox = create_overlay(image, heatmap, top_label, top_conf,
